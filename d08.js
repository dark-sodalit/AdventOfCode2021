function d08() {
    ...
    var data = d08_input.split('\n');
    part1.innerText = d08_part1([...data]);
    part2.innerText = d08_part2([...data]);
}

function d08_part1(data) {
    data = data.map(value => value.split(' | ')[1].split(' '));
    var uniqueSum = 0;
    for (var i = 0; i < data.length; i++)
        for (var j = 0; j < data[i].length; j++)
            switch (data[i][j].length) {
                case 2:
                case 3:
                case 4:
                case 7:
...
function d08_part2(data) {
    var sum = 0;
    for (var i = 0; i < data.length; i++) {
        data[i] = data[i].split(' | ');
        var pattern = data[i][0].split(' ');
        var output = data[i][1].split(' ');
        var digits = [];
        for (var j = 0; j < pattern.length; j++) {
            ...
            if (digit.length == 5) {
                if (!digits[5])
                    digits[5] = [];
                digits[5][digits[5].length] = digit;
            }
            else if (digit.length == 6) {
                if (!digits[6])
                    digits[6] = [];
                digits[6][digits[6].length] = digit;
            }
            else
        ...
        for (var j = 0; j < 3; j++) {
            var char = digits[3][j];
            if (digits[2].indexOf(char) === -1)
                a = char;
        }
        var d;
        for (var j = 0; j < 4; j++) {
            var char = digits[4][j];
            if (digits[5][0].indexOf(char) !== -1 && digits[5][1].indexOf(char) !== -1 && digits[5][2].indexOf(char) !== -1)
                d = char;
        }
        var g;
        for (var j = 0; j < 5; j++) {
            var char = digits[5][0][j];
            if (digits[5][1].indexOf(char) !== -1 && digits[5][2].indexOf(char) !== -1 && char !== a && char !== d)
                g = char;
        }
        var f;
        for (var j = 0; j < 6; j++) {
            var char = digits[6][0][j];
            if (digits[2].indexOf(char) !== -1 && digits[6][1].indexOf(char) !== -1 && digits[6][2].indexOf(char) !== -1)
                f = char;
        }
        var c = (digits[2][0] == f ? digits[2][1] : digits[2][0]);
        var result = 0;
